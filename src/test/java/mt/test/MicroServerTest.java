...
public class MicroServerTest {
	...
	public void testStart() throws Exception {
		when(serverComm.getNextMessage()).thenReturn(msg1).thenReturn(msg2).thenReturn(msg3).thenReturn(msg4)
				.thenReturn(msg5).thenReturn(msg6).thenReturn(null);
		...
		verify(serverComm, atLeastOnce()).sendOrder("userA", Order.createBuyOrder("userB", "MSFT", 10, 21.0));
	...
	public void testStart2() throws Exception {
		...
		verify(serverComm, atLeastOnce()).sendError(msg2.getSenderNickname(),
				"The user " + msg2.getSenderNickname() + " is not connected.");
	}
	...
	public void testStartProcessSellOrder() throws Exception {
		when(serverComm.getNextMessage()).thenReturn(msg3).thenReturn(msg8).thenReturn(msg1).thenReturn(msg9)
				.thenReturn(null);
	...
	public void testStartProcessBuyOrder() throws Exception {
		...
		verify(serverComm, atLeastOnce()).sendOrder("userA", Order.createSellOrder("userB", "MSFT", 10, 21.0));
	...
	public void testUserAlreadyConnected() throws Exception {
		...
		verify(serverComm, atLeastOnce()).sendError(msg1.getSenderNickname(),
				"The user " + msg1.getSenderNickname() + " is already connected.");
	}
