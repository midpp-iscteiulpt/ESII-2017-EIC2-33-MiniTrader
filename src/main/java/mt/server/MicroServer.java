...
public class MicroServer implements MicroTraderServer {
	...
	public void start(ServerComm serverComm) {
		...
		while ((msg = serverComm.getNextMessage()) != null) {
			...
			case DISCONNECTED:
				...
				break;
			case NEW_ORDER:
				if (invalidOrderEU(msg.getOrder()))
					break;
				try {
	...
	private boolean invalidOrderEU(Order order) {
		...
		// clients are not allowed to issue sell orders for their own buy orders
		// and vice versa
		// if order is sell order
		if (order.isSellOrder()) {
			for (Entry<String, Set<Order>> entry : orderMap.entrySet()) {
				for (Order o : entry.getValue()) {
					if (o.isBuyOrder() && o.getNickname().equals(order.getNickname())
							&& o.getStock().equals(order.getStock())) {
						return true;
					}
				}
			}
		...
		if (order.isBuyOrder()) {
			for (Entry<String, Set<Order>> entry : orderMap.entrySet()) {
				for (Order o : entry.getValue()) {
					if (o.isSellOrder() && o.getNickname().equals(order.getNickname())
							&& o.getStock().equals(order.getStock())) {
						return true;
					}
				}
